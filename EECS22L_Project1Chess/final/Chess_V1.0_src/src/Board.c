...
MoveList GetAllMoves(Color teamColor, Board *board) {
            ...
            if (board->grid[currentStartingPosition.file][currentStartingPosition.rank].color == teamColor)
            {
                for (rank = one; rank <= eight; rank++) {
                    ...
                    for (file = a; file <= h; file++) {
                        ...
                        //printf("Calling Rules() function...\n");
                        //PrintFileAndRank(currentStartingPosition);
                        //PrintFileAndRank(currentFinalPosition);
                        errorcode = Rules(currentStartingPosition, currentFinalPosition, board);
                        //printf("done using Rules()...\n");
                        if (errorcode == 0) {
                            //printf("adding move into list\n");
    ...
    //printf("done iterating through for-loop in GetMoves()...\n");

    listOfMoves.size = totalMoves;
    //printf("list of possibe moves size: %d\n", totalMoves);
    ...
    //printf("returning list of moves...\n");
